...
from diagnostico_v2_common import ART, ROOT, V2

V3_ART = ROOT / "ia-service/artifacts/diagnostico_v3"
HEAT = V3_ART / "heatmaps_v3"


def heatmap_stats(cam_path, bbox_norm):
...
def main():
    ...
    ap.add_argument("--salida", default="heatmap_stats.json")
    args = ap.parse_args()

    report = json.loads((V3_ART / f"heatmaps_report_{args.tag}.json").read_text(encoding="utf-8"))
    stems = {
    ...
    for (cls, tag), stem in stems.items():
        ...
        if not heat_path.exists():
            continue
        st = heatmap_stats(heat_path, bbox)
        rows.append({"caso": f"{cls}_{tag}", "bbox_gt": str(bbox),
                     "predicciones": entry.get("predicciones"), **st})
    for r in rows:
        print(r)
    (V3_ART / args.salida).write_text(json.dumps(rows, indent=2, ensure_ascii=False), encoding="utf-8")
