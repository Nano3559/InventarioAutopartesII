...
HEATMAP_DIR = V3_ART / "heatmaps_v3"
...
def casos():
    ...
    dic["alternator"]["degradada_perspectiva"] = \
        next(iter(HOLDOUT.glob("kaggle_prod__alternator__test__1__C_perspectiva_moderada.png")), None)
    dic["alternator"]["degradada_webcam"] = \
        next(iter(HOLDOUT.glob("kaggle_prod__alternator__test__5__K_webcam_realista.png")), None)
    dic["alternator"]["vacia_escala"] = \
        next(iter(HOLDOUT.glob("kaggle_prod__alternator__test__1__J_escala_45.png")), None)
    dic["alternator"]["vacia_webcam"] = \
        next(iter(HOLDOUT.glob("kaggle_prod__alternator__val__2__K_webcam_realista.png")), None)
    dic["radiator"]["degradada"] = \
        next(iter(HOLDOUT.glob("kaggle_prod__radiator__test__1__E_blur_moderado.png")), None)
    dic["headlight"]["degradada"] = \
        next(iter(HOLDOUT.glob("kaggle_prod__headlight__test__1__K_webcam_realista.png")), None)
    return dic
    ...
    with EigenCAMExtractor(model, TARGET_LAYERS) as ex:
        for cls, items in casos_sel.items():
            ...
            for tag, p in items.items():
                ...
                preds = []
                for d in dets:
                    preds.append({"clase": CLASSES[int(d.cls.item())],
                ...
                (HEATMAP_DIR / f"{cls}__{tag}__report.json").write_text(
                    json.dumps({"modelo": args.modelo, "clase": cls, "tag": tag, "predicciones": preds,
                                "heatmaps": out}, indent=2, ensure_ascii=False), encoding="utf-8")
                report[cls][tag] = {"imagen": p.name, "predicciones": preds, "heatmaps": out}
                print(cls, tag, p.name, "->", [(c["clase"], c["conf"]) for c in preds])
    (V3_ART / f"heatmaps_report_{args.tag}.json").write_text(
