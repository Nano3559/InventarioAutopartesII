...
export default function LandingPage() {
  ...
  const handleCatalog = () => navigate("/productos");
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {[
              { icon: Settings, name: "Amortiguadores Monroe", brand: "Toyota / Nissan", price: "Bs. 320", oldPrice: "Bs. 450", badge: "Más vendido", badgeStyle: "text-amber-400 bg-amber-500/10 border-amber-500/20", accent: "group-hover:text-amber-400" },
              { icon: Wrench, name: "Filtro de Aceite Mann", brand: "Universal", price: "Bs. 45", oldPrice: null, badge: "Disponible", badgeStyle: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20", accent: "group-hover:text-emerald-400" },
              { icon: Zap, name: "Pastillas de Freno Brembo", brand: "Mazda / Nissan", price: "Bs. 280", oldPrice: "Bs. 350", badge: "Nuevo", badgeStyle: "text-blue-400 bg-blue-500/10 border-blue-500/20", accent: "group-hover:text-blue-400" },
              { icon: CircleDollarSign, name: "Correa de Distribución", brand: "Toyota Hilux", price: "Bs. 190", oldPrice: null, badge: "Recomendado", badgeStyle: "text-violet-400 bg-violet-500/10 border-violet-500/20", accent: "group-hover:text-violet-400" },
            ].map((product) => (
              <div
                key={product.name}
              ...
              >
                <div className="relative w-full aspect-square bg-dark-800/60 rounded-xl mb-5 flex items-center justify-center overflow-hidden border border-white/[0.03]">
                  ...
                  <product.icon size={44} strokeWidth={1.2} className={`text-gray-600/60 ${product.accent} transition-colors duration-300 relative z-10`} />
                </div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  {product.oldPrice && <span className="text-xs text-gray-500 line-through">{product.oldPrice}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-28 bg-dark-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-400 max-w-2xl mx-auto">Soluciones integrales para el mantenimiento de tu flota vehicular.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Package, title: "Amplio Inventario", desc: "Miles de repuestos para diferentes marcas y modelos de vehículos disponibles en stock." },
              { icon: Truck, title: "Distribución Rápida", desc: "4 almacenes y 3 tiendas para garantizar que recibas tu pedido a tiempo." },
              { icon: Shield, title: "Calidad Garantizada", desc: "Trabajamos con los mejores proveedores para ofrecer productos de primera calidad." },
            ].map((item) => (
              <div key={item.title} className="group bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8 hover:bg-white/[0.04] hover:border-white/[0.1] transition-all duration-300">
      ...
      <section className="py-24 bg-dark-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              { icon: MapPin, title: "Dirección", lines: ["Av. Principal #1234", "Cochabamba, Bolivia"] },
              { icon: Phone, title: "Teléfono", lines: ["+591 4 1234567", "+591 71234567"] },
              { icon: Clock, title: "Horario", lines: ["Lunes a Viernes: 8:30 - 18:00", "Sábados: 8:30 - 13:00"] },
            ].map((item) => (
              <div key={item.title} className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8 text-center">
                ...
                {item.lines.map((line, i) => <p key={i} className="text-gray-400 text-sm">{line}</p>)}
              </div>
            ))}
          </div>
        </div>
